...
public class WeatherManager: WeatherManaging {
    private let networkManager = Network.shared
    
    public init() {}
    
    public func getCurrentWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> CurrentResponse {
    ...
    public func getForecastWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> ForecastResponse {
    ...
    public func request<T: Decodable>(router: Router) async throws -> T {
